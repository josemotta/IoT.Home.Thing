...
- **Lirc**: The IoT.Home.Thing device is enabled to handle legacy IR remote controlled gadgets. The Linux Infrared Remote Control for Raspberry Pi is installed at both RPI host and `home-srv`, extending the `Thing` API and respective web service to support IR remotes and their IR codes.
