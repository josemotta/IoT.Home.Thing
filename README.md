...
- **Lirc**: The IoT.Home.Thing device is enabled to handle legacy IR remote controlled gadgets. The Linux Infrared Remote Control for Raspberry Pi is installed at RPI host and `home-srv`, extending the `Thing` web service to support IR remotes and their respective IR codes.
